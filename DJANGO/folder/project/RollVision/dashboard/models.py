...
class Student(models.Model):
    ...
    roll_number = models.CharField(max_length=10, default='TEMP', help_text="Roll number within division")
    name = models.CharField(max_length=100)
    ...
    phone_number = models.CharField(validators=[phone_regex], max_length=17, default='0000000000', help_text="Primary contact number")
    secondary_phone = models.CharField(validators=[phone_regex], max_length=17, blank=True, null=True, help_text="Optional secondary contact number")
    ...
    photo = models.ImageField(
        ...
        null=True,
        blank=True,
        help_text="Student photo for face recognition (alternative to camera capture)"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['class_year', 'division']),
            models.Index(fields=['is_trained', 'is_active']),
        ]


class FaceEncoding(models.Model):
    """Store face encoding data for each student"""
    ...
    image_path = models.CharField(max_length=500, help_text="Path to stored face image")
    created_at = models.DateTimeField(auto_now_add=True)
    is_active = models.BooleanField(default=True)

    def __str__(self):
        return f"Face encoding for {self.student.name}"
...
class AttendanceRecord(models.Model):
    """Track student attendance records with lecture-based tracking"""
    STATUS_CHOICES = [
        ('present', 'Present'),
        ('absent', 'Absent'),
        ('late', 'Late'),
    ...
    subject = models.ForeignKey(
        ...
        null=True,
        blank=True,
        help_text="Subject for which attendance was marked"
    )
    lecture_period = models.ForeignKey(
        LecturePeriod,
        on_delete=models.CASCADE,
        related_name='attendance_records',
        null=True,
        blank=True,
        help_text="Lecture period when attendance was marked"
    )
    ...
    confidence_score = models.FloatField(null=True, blank=True, help_text="Face recognition confidence (0-1)")
    
    # For backward compatibility - keep time field
    time = models.TimeField(auto_now_add=True)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        subject_code = self.subject.code if self.subject else 'N/A'
        return f"{self.student.name} - {subject_code} - {self.date} - {self.status}"

    class Meta:
        ordering = ['-date', '-marked_at']
        indexes = [
            models.Index(fields=['-date', '-marked_at']),
